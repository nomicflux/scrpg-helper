...
object SceneTracker:
  ...
  def sceneTracker(challengeCreator: ChallengeCreatorModel): Element =
    div(
      ...
      div(
        h3("Challenges"),
        className := "challenge-holder",
        className <-- challengeCreator.challengesSignal.map(cs =>
          s"challenges-${cs.size > 0}"
        ),
        div(
      ...
      table(
        tr(
          children <-- model.sceneSignal.map { scene =>
            ...
            statusBoxes
              .zip(statusBoxes.indices)
              .map((status, position) =>
                renderSceneBox(status, position + 1, scene.position)
              )
          }
        )
      )
  ...
  def renderActorList(
      getActors: ActorQueue[Actor] => Seq[Actor],
      withEmpty: Boolean
  ): Element =
    ul(
      children <-- model.actorsSignal.map { q =>
        ...
        actors
          .map(renderActor) ++ (if withEmpty then
                                  renderEmpty(q.totalActors.size, actors.size)
                                else List())
      }
    )
  end renderActorList
  ...
  def addActorInput(): Element =
    ...
    def addActor(at: ActorType, acted: Boolean): Unit =
      actorName.update { name =>
        if (acted) then
          model.actorActedAdder.onNext(Actor.createActor(at, name))
        else model.actorAdder.onNext(Actor.createActor(at, name))
        ""
      }
    end addActor
    ...
    div(
        ...
        input(
          ...
          onKeyPress.compose(
            _.withCurrentValueOf(
              actorTypeSignal.combineWith(actorAlreadyActedSignal)
            )
          ) --> { case (event, at, acted) =>
            if (event.key == "Enter") {
              addActor(at, acted)
      ...
      span(
        select(
          ...
          onChange.mapToValue
            .compose(_.withCurrentValueOf(dieSizeSignal)) --> { case (s, d) =>
            ActorType.fromString(s, d).foreach(at => actorType.update(_ => at))
          }
        ),
        select(
          className <-- actorTypeSignal.map(at =>
            s"select-die-size actor-has-die-${at.hasDie}"
          ),
          option(value := "4", "d4"),
          option(value := "6", "d6"),
          option(value := "8", "d8"),
          option(value := "10", "d10"),
          option(value := "12", "d12"),
          onChange.mapToValue.map(_.toIntOption).collect { case Some(n) =>
            n
          } --> { n =>
            dieSize.update(_ => n)
  ...
  def renderBoxUpdater(): Element =
    div(
      ...
      NumBox(model.greenSignal, model.greenIncrementer, model.greenUpdater)
        .withLabel("Green")
        .withSpanClassName("status-updater-span")
        .withInputClassName("status-updater status-green")
        .withMinVal(0)
        .render(),
      NumBox(model.yellowSignal, model.yellowIncrementer, model.yellowUpdater)
        .withLabel("Yellow")
        .withSpanClassName("status-updater-span")
        .withInputClassName("status-updater status-yellow")
        .withMinVal(0)
        .render(),
      NumBox(model.redSignal, model.redIncrementer, model.redUpdater)
        .withLabel("Red")
        .withSpanClassName("status-updater-span")
        .withInputClassName("status-updater status-red")
        .withMinVal(0)
        .render()
    )
...
final class SceneTrackerModel:
  ...
  def optSceneUpdater[A](
      f: (Scene[Actor], A) => Option[Scene[Actor]]
  ): Observer[A] =
  ...
  val advanceTracker: Observer[Unit] =
    optSceneUpdater((scene, _) => scene.advancePosition)
  val resetScene: Observer[Unit] = sceneUpdater((scene, _) => scene.reset)

  val actorAdder: Observer[Actor] =
    sceneUpdater((scene, actor) => scene.addActor(actor))
  val actorActedAdder: Observer[Actor] =
    sceneUpdater((scene, actor) => scene.addActedActor(actor))
  val actorRemover: Observer[Actor] =
    sceneUpdater((scene, actor) => scene.removeActor(actor))

  val greenUpdater: Observer[Int] =
    optSceneUpdater((scene, green) => scene.updateGreen(green))
  val yellowUpdater: Observer[Int] =
    optSceneUpdater((scene, yellow) => scene.updateYellow(yellow))
  val redUpdater: Observer[Int] =
    optSceneUpdater((scene, red) => scene.updateRed(red))

  val greenIncrementer: Observer[Int] =
    optSceneUpdater((scene, change) => scene.updateGreen(scene.green + change))
  val yellowIncrementer: Observer[Int] = optSceneUpdater((scene, change) =>
    scene.updateYellow(scene.yellow + change)
  )
  val redIncrementer: Observer[Int] =
    optSceneUpdater((scene, change) => scene.updateRed(scene.red + change))

  val actorAdvancer: Observer[Actor] = optSceneUpdater { (scene, actor) =>
    val advanced = scene.advanceScene(actor)
    if actor.actorType.advancesScene() then advanced.flatMap(_.advancePosition)
    else advanced
  }
  val actorDieIncreaser: Observer[Actor] = optSceneUpdater((scene, actor) =>
    scene.updateActor(actor, a => a.increaseDieSize())
  )
  val actorDieDecreaser: Observer[Actor] = optSceneUpdater((scene, actor) =>
    scene.updateActor(actor, a => a.decreaseDieSize())
  )
