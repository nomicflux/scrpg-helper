...
case class AbilityTemplate(
  ...
  val baseChoices: List[AbilityChoice] = description.collect {
    case ac: AbilityChoice => ac
  }
...
case class ChosenAbility(
  ...
  override def toString(): String =
    s"ChosenAbility(${template.name}, ${template.status}, ${template.category}, ${inPool.max}/${inPool.abilities.size}, ${currentChoices
        .map(_.toString)} Valid: ${valid}})"
  ...
  def descriptionFilledOut: Boolean = description
    .collect { case ac: AbilityChoice => ac}
    .filter(_.getItem.isEmpty)
    .isEmpty

  def applyChoice(choice: AbilityChoice): ChosenAbility =
    copy(currentChoices =
      currentChoices.map(c => if c.id == choice.id then choice else c)
    )
  end applyChoice
  ...
  def removeChoice(choice: AbilityChoice): ChosenAbility =
    copy(currentChoices =
      currentChoices.map(c =>
        if c.id == choice.id then choice.withoutChoice else c
      )
    )
  end removeChoice
