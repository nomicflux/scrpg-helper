...
object Background:
    ...
    val lawEnforcement: Background = Background(
      "Law Enforcement",
      6,
      List(Q.closeCombat, Q.criminalUnderworldInfo, Q.rangedCombat) ++ Q.mentalQualities ++ Q.socialQualities,
      PC.Responsibility,
      List(d(10), d(8)),
      List(d(10), d(8), d(6))
    )
    ...
    val backgrounds: List[Background] = List(
      upperClass, blankSlate, struggling, adventurer, unremarkable, lawEnforcement, academic,
      tragic, performer, military, retired, criminal, medical, anachronistic, exile,
      formerVillain, interstellar, dynasty, otherworldly, created
    )
end Background
